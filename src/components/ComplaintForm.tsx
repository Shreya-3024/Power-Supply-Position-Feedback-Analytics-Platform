...
function StepOne({ formData, errors, handleChange, nextStep }: any) {
  return (
    <motion.div
      key="step1"
...
function StepTwo({ formData, errors, handleChange, setFormData, nextStep, prevStep }: any) {
  return (
    <motion.div
      key="step2"
      initial={{ opacity: 0, x: 50 }}
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            {ISSUE_TYPES.map((type) => (
              <button
                key={type}
                type="button"
                onClick={() => setFormData((prev: any) => ({ ...prev, issueType: type }))}
...
function StepThree({ formData, setFormData, prevStep, loading }: any) {
  return (
    <motion.div
      key="step3"
      initial={{ opacity: 0, x: 50 }}
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {PRIORITIES.map((priority) => (
              <button
                key={priority.value}
                type="button"
                onClick={() => setFormData((prev: any) => ({ ...prev, priority: priority.value }))}
                className={`p-4 rounded-xl border-2 transition-all ${
...
function InputField({ label, name, type = 'text', value, onChange, error, placeholder, required = false }: any) {
  return (
    <div>
      <label className="block text-sm text-slate-700 dark:text-slate-300 mb-2">
